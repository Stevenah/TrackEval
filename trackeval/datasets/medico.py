...
class Medico(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'CLASSES_TO_EVAL': ['sperm', 'pinhead', 'cluster'],  # Valid: ['sperm', 'pinhead', 'cluster']
            'BENCHMARK': 'Medico',  # Valid: 'Medico'
            'CONVERT_FROM_YOLO': True,  # Whether boudning box coordinats use the yolo format or not
    ...
    def __init__(self, config=None):
        ...
        # Get classes to eval
        self.valid_classes = ['sperm', 'pinhead', 'cluster']
        self.class_list = [cls.lower() if cls.lower() in self.valid_classes else None
                           for cls in self.config['CLASSES_TO_EVAL']]
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class.')
        self.class_name_to_class_id = { 'sperm': 0, 'pinhead': 1, 'cluster': 2 }
